...
def read_one_map(path, norm=True, reshape_shape=None):
    tmp = hp.read_map(path) if not norm else normalize(hp.read_map(path))
    # replace nan with 0
    tmp[np.isnan(tmp)] = 0
...
class MapDataset(data.Dataset):
    ...
    """
    def __init__(self, lrdir, hrdir, n_maps=None, norm=True, order=None, difference=True,  verbose=True):
        if n_maps is None:
        ...
        if order is not None:
            ...
            self.hrmaps = [read_one_map(path, norm=norm, reshape_shape=[self.n_patches, -1, 1]) for path in self.hrmap_path]
            ...
            self.hrmaps = [read_one_map(path, norm=norm, reshape_shape=[-1, 1]) for path in self.hrmap_path]
