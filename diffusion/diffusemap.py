...
if __name__ == '__main__':
    ...
    ckpt_path= "/gpfs02/work/akira.tokiwa/gpgpu/Github/SR-SPHERE/srsphere/diffusion/lightning_logs/version_8/checkpoints/Run_09-25_11-12epoch=307-val_loss=0.03.ckpt"
    ...
    data_hr, data_lr = get_datasets(map_dirs, n_maps, nsides, config_dict['data']['order'], config_dict['data']['issplit'], config_dict['data']['normalize'])
    ...
    q_sample = tmp_diffusion.q_sample(tmp_sample, torch.full((48,), timesteps-1, device=device))
    img = torch.randn(tmp_sample.shape, device=device)
    for j in reversed(range(0, timesteps//2)):
        t = torch.full((48,), j, device=device, dtype=torch.long)
        ...
        if j % 10 == 0:
            ...
            hp.write_map(map_dir+"diffused_step{}.fits".format(j), diffmap, overwrite=True, dtype=np.float32)
    diffmap = np.hstack(inverse_transforms(img).detach().cpu().numpy()[:48, : , 0])
    hp.write_map(map_dir+"diffused_step{}.fits".format(j), diffmap, overwrite=True, dtype=np.float32)